...
public:
  ...
  VMEBoardReader(int boardNumber, TString pathName, TString prefixName, TString suffixName, int runNumber, int numberOfSensors, int numberOfRows, Int_t ApplyVeto);
  ~VMEBoardReader();
...
private:
  ...
  Int_t           fApplyVeto;      // if = 1, remove pixels in the list veto.c  RS 11/02/20
  int             fVetoOverflow;
