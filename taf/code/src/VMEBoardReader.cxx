...
//------------------------------------------+-----------------------------------
//! Default constructor.
VMEBoardReader::VMEBoardReader(int boardNumber, TString pathName, TString prefixName, TString suffixName, int runNumber, int numberOfSensors, int numberOfRows, Int_t ApplyVeto) {
...
/*
// --------------------------------------------------------------------------------------
bool vetoPixel( int chipNb, int row, int col) //26166
{
...
void VMEBoardReader::AddPixel( Int_t iSensor, Int_t value, Int_t aLine, Int_t aColumn)
{
   ...
   if( fApplyVeto == 1 ){//if fApplyVeto == 1, check if pixels belong to the list of noisy pixels of veto.c, to remove them
        if( !veto(iSensor, aLine, aColumn) ) ListOfPixels.push_back( BoardReaderPixel( iSensor+1, value, aLine, aColumn, 0) );
   }
   else{// do not remove noisy pixels
