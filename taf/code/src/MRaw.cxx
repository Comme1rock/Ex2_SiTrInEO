...
//      bar3->AddButton("RAW CHANNELS", "gTAF->GetRaw()->DisplayRawData(1.)", "Display raw channels per plane and event");
      ...
      bar3->AddButton("CUMULATE RAW CHANNELS 2D", "gTAF->GetRaw()->DisplayCumulatedRawData2D(150000)", "Display raw data per plane cumulated over 150 000 events in 2D");
      bar3->AddButton("HOT PIXELS MAP", "gTAF->GetRaw()->DisplayCumulatedRawData2D(150000, \"continuous\", -65536, 0.001, 1, 20, 0, true)", "Display hot pixels map per plane over 150 000 events");
      bar3->AddButton("HITS 2D", "gTAF->GetRaw()->DisplayHits2D(2,1,0)", "Display hits per plane and event in 2D");
      bar3->AddButton("CUMULATE HITS 2D", "gTAF->GetRaw()->DisplayCumulatedHits2D(150000)", "Display hits per plane cumulated over 150 000 events in 2D");
      ...
      if( strstr(goal, "sitrineo") || strstr(goal, "Sitrineo") ) {
        ...
        bar3->AddButton("SITRINEO/EVT W/ 4 HITS", "gTAF->GetRaw()->Sitrineo_4Hits(2)", "Sitrineo analysis event-by-event, with at least 4 hits by plane");
        bar3->AddButton("SITRINEO/CUMUL", "gTAF->GetRaw()->SitrineoCumul(150000,2)", "Sitrineo analysis cumulated over 100 000 events");
        bar3->AddButton("TRACKS", "gTAF->GetRaw()->DisplayTracks()", "Display tracks per event in several views");
        bar3->AddButton("CUMULATE TRACKS", "gTAF->GetRaw()->DisplayCumulatedTracks()", "Display tracks cumulated over 500 events in several views");
        bar3->AddButton("HITS-TRACKS HISTORY", "gTAF->GetRaw()->DisplayHistory()", "Display hits and tracks properties wrt event#");
        bar3->AddButton("Study Beam Profile", "gTAF->GetRaw()->StudyBeamProfile(1000)", "Study Beam Profile");
      ...
      bar3->AddButton("SKIP 10 000 EVENTS", "gTAF->GetRaw()->SkipEvent(10000)", "Skip 10 000 events without analyzing them for hits");
				     ...
				     double Linmin, double Linmax)
{
  ...
  int sum[4] ; sum[0] = 0 ; sum[1] = 0 ; sum[2] = 0 ; sum[3] = 0 ; //vector containing the number of fired pixels on each plane
  for( Int_t iEvt=0; iEvt < nEvents; iEvt++) {
  ...
  //print the number of fired pixels
  for( int iPlane = 1 ; iPlane <= nPlanes ; iPlane++){
      cout << "Plane " << iPlane << " has seen " << sum[iPlane-1] << " fired pixels." << endl;
  }
  ...
  if( storeOccurence ) {
    ...
    //create and write the file veto.C, containing the list of noisy pixels
    TString veto_fileName =  gSystem->GetFromPipe("echo $DTDIR") + (TString)"/code/src/veto.c" ;// 
    FILE *veto = fopen(veto_fileName, "w");
    fprintf(veto, "bool veto( int chipNb, int row, int col){\n");
    fprintf(veto, " return \n");
    bool first = true ;
      ...
      for( Int_t xBin=0; xBin<hRDMap[iPlane-1]->GetNbinsX(); xBin++) { // loop on X bins
        for( Int_t yBin=0; yBin<hRDMap[iPlane-1]->GetNbinsY(); yBin++) { // loop on Y bins
          ...
          if( occurence > minOccurenceForFile ) {
            ...
            //Write the list of noisy pixels
            if(first){
                fprintf(veto, "     (chipNb==%d && row==%4d && col==%4d)\n", iPlane-1 , yBin, xBin); 
                first = false ;
            }
            else{ 
                fprintf(veto, "     || (chipNb==%d && row==%4d && col==%4d)\n", iPlane-1, yBin, xBin) ;
...
// == Plane, hit and track info
  ...
  //Canvas and pads for hits from different planes
  TCanvas *cHitsAlign = new TCanvas("HitsAlign", "HitsAlign", 0, 0, 900, 900);
  TPad *padHitsAlign = new TPad("padHits","",0.,0.,1.,0.965);
  padHitsAlign->Draw();
...
//        printf(" hit %d, (col,row)=(%3d, %3d), (u,v)=(%.1f, %.1f), Npixels=%d, testMult=%d\n", iHit, (int)TheCol, (int)TheRow, u, v, Npixels_in_Hit, testMult);
    ...
    //display for align hits
    cHitsAlign->cd();
    padHitsAlign->cd();
    if(iPlane == 1){
        hHitMapTel[iPlane-1]->SetMarkerColor(iPlane);
        hHitMapTel[iPlane-1]->DrawCopy("");
    }
    else{
        hHitMapTel[iPlane-1]->SetMarkerColor(iPlane);
        hHitMapTel[iPlane-1]->DrawCopy("same");
    }
...
void MRaw::SitrineoCumul( Int_t nEvents, Int_t lastPlaneOfFirstTracker)
{
  ...
  TCanvas *cAtLeastHitPerEvent = new TCanvas(">= 1 hits/plane/evt", ">= 1 hits/plane/evt", 0, 0, 800, 700);
  TPad *padAtLeastHitPerEvent = new TPad("padAtLeastHitPerEvent","",0.,0.,1.,0.965);
  padAtLeastHitPerEvent->Draw();
  
  TCanvas *cHitsPerEvent = new TCanvas("Exactly 4 hits", "Exactly 4 hits", 0, 0, 800, 700);
  TPad *padHitsPerEvent = new TPad("padHitsPerEvent","",0.,0.,1.,0.965);
  padHitsPerEvent->Draw();
  
  TCanvas *cSpectrum = new TCanvas("Mom Spectrum", "Mom Spectrum", 0, 0, 800, 700);
  TPad *padSpectrum = new TPad("padSpectrum","",0.,0.,1.,0.965);
  padSpectrum->Draw();
  
  TCanvas *cMultScatt = new TCanvas("hMultScatt", "hMultScatt", 0, 0, 1800, 900);
  //TPad *padMultScattX = new TPad("padMultScattX","",0.,0.,1.,0.965);
  //padMultScattX->Draw();
  ...
  TH1F **hNHitsVSEventN = new TH1F*[nPlanes]; // QL 2015/10/23
  // RS 10/02/2020
  TH1D *hAtLeastHitPerEvent = new TH1D("hatleasthitpplpevt", ">=1 Hit per plane per event", nEvents, 0, nEvents);
  TH1D *hHitsPerEvent = new TH1D("h4hitpplpevt", "1 Hit per plane per event", nEvents, 0, nEvents);
  TH1D *hSpectrum = new TH1D("hSpect4hitspplpevt", "Mom Spectrum with 1 Hit per plane per event", 100, 0, 10);
  TH1D *hMultScattX_Pl1 = new TH1D("hMultScattX_Pl1", "Deviation due to Mult Scatt along X in Pl1", 300, -30000, 30000);
  hMultScattX_Pl1->GetXaxis()->SetLabelSize(0.03);
  TH1D *hMultScattY_Pl1 = new TH1D("hMultScattY_Pl1", "Deviation due to Mult Scatt along Y in Pl1", 300, -30000, 30000);
  hMultScattY_Pl1->GetXaxis()->SetLabelSize(0.03);
  TH1D *hMultScattX_Pl2 = new TH1D("hMultScattX_Pl2", "Deviation due to Mult Scatt along X in Pl2", 300, -30000, 30000);
  hMultScattX_Pl2->GetXaxis()->SetLabelSize(0.03);
  TH1D *hMultScattY_Pl2 = new TH1D("hMultScattY_Pl2", "Deviation due to Mult Scatt along Y in Pl2", 300, -30000, 30000);
  hMultScattY_Pl2->GetXaxis()->SetLabelSize(0.03);
  TH1D *hMultScattX_Pl3 = new TH1D("hMultScattX_Pl3", "Deviation due to Mult Scatt along X in Pl3", 300, -30000, 30000);
  hMultScattX_Pl3->GetXaxis()->SetLabelSize(0.03);
  TH1D *hMultScattY_Pl3 = new TH1D("hMultScattY_Pl3", "Deviation due to Mult Scatt along Y in Pl3", 300, -30000, 30000);
  hMultScattY_Pl3->GetXaxis()->SetLabelSize(0.03);
  ...
  for( Int_t iPlane=1; iPlane<=nPlanes; iPlane++) {
    ...
    hHitMap[iPlane-1] = new TH2F(name, title, NbinsX, xmin, xmax, NbinsY, ymin, ymax);
    hHitMap[iPlane-1]->GetXaxis()->SetLabelSize(0.03);
    hHitMap[iPlane-1]->GetYaxis()->SetLabelSize(0.03);
    hHitMap[iPlane-1]->SetMarkerStyle(20);
    hHitMap[iPlane-1]->SetMarkerSize(.1);
    hHitMap[iPlane-1]->SetMarkerColor(1);
    hHitMap[iPlane-1]->SetStats(kFALSE);
  ...
  for( Int_t iEvt=0; iEvt < nEvents; iEvt++) {
    ...
    bool a = true ;//bool for at least one hit per sensor 
    bool b = true ;//bool for exactly one hit per sensor
    bool c = true ;//bool for exactly one hit on sensor 3 and 4 ( the first two sensors hit )
    bool d = true ;//bool for exactly one hit on sensor 2, 3 and 4 ( the first three sensors hit )
    bool e = true ;//bool for exactly one hit on sensor 1, 2 and 3 ( the last three sensors hit )
    int s[nPlanes] ; //table containing the number of hits on a given plane
    double posHitX[nPlanes] ; double posHitY[nPlanes] ; double posHitZ[nPlanes] ; //table containing the hit position on a given plane
    
    for( Int_t iPlane=1; iPlane<=nPlanes; iPlane++) { // loop on planes
      tPlane = tTracker->GetPlane(iPlane);
      hNHitsPerEvent[iPlane-1]->Fill( tPlane->GetHitsN() );
      s[iPlane-1] = tPlane->GetHitsN() ;
      ...
      if( s[iPlane-1] == 1 ){//if exactly 1 hit per plane
          DHit* aHit = (DHit*)tPlane->GetHit(1) ;
          posHitX[iPlane-1] = tPlane->PlaneToTracker(*(aHit->GetPosition()))(0) ;
          posHitY[iPlane-1] = tPlane->PlaneToTracker(*(aHit->GetPosition()))(1) ;
          posHitZ[iPlane-1] = tPlane->PlaneToTracker(*(aHit->GetPosition()))(2) ;
      }
      
      a&=( s[iPlane-1] >= 1) ;//if at least 1 hit per plane

      b&=( s[iPlane-1] == 1) ;//if exactly 1 hit per plane
      
      
    } //end loop on planes
    
    c&=(s[2] == 1 && s[3] == 1 ) ;//if exactly 1 hit on plane 3 and 4
    
    d&=(s[1] == 1 && s[2] == 1 && s[3] == 1) ;//if exactly 1 hit on plane 2, 3 and 4
    
    e&=(s[0] == 1 && s[1] == 1 && s[2] == 1) ;//if exatcly 1 hit on plane 1, 2 and 3
        
    if(a){ hAtLeastHitPerEvent->Fill(iEvt);}
    
    if(b){
        //double theta = TMath::ATan( ( (posHitX[1] - posHitX[0])/(posHitZ[1]-posHitZ[0]) ) )  - TMath::ATan( (posHitX[3] - posHitX[2])/(posHitZ[3] - posHitZ[2]) );
        //double p = 0.3*1*10/abs(theta);
        //std::cout << "theta = " << theta << " ; p = " << p << std::endl ;
        //hSpectrum->Fill(p);
        hHitsPerEvent->Fill(iEvt);
        
    }
    
    if(c){//impact of multiple scattering on sensor 3
        double x4 = posHitX[3] ; double y4 = posHitY[3] ; double z4 = posHitZ[3] ;
        double x3_MS = posHitX[2] ; double y3_MS = posHitY[2] ; double z3 =  posHitZ[2] ;
        double l = 270E3 ;//micrometer, distance between first sensor and source
        double xS = 1602 ; double yS = 7018 ; double zS = posHitZ[3] - l ; //values of xS and yS are coming from the analysis done with the macro result.C. Extrapolation of the position of the image of the source on the two reference sensors allows us to access the position of the source
        
        //calculate the position of the hit if there would be no multiple scattering, ie linear extrapolation based on source position and hit position on sensor 4
        double x3 = z3*(x4-xS)/(z4 - zS) + (xS*z4- x4*zS)/(z4-zS);
        double y3 = z3*(y4-yS)/(z4 - zS) + (yS*z4- y4*zS)/(z4-zS);
        
        //Impact of multiple scattering is therefore given by the difference between the observed hit position ( with multiple scattering ) minus the extrapolated hit position (if there would be no multiple scattering)
        hMultScattX_Pl3->Fill(x3_MS - x3);
        hMultScattY_Pl3->Fill(y3_MS - y3);
    }
    
    if(d){//impact of multiple scattering on sensor 2
        double x4 = posHitX[3] ; double y4 = posHitY[3] ; double z4 = posHitZ[3] ; 
        double x3 = posHitX[2] ; double y3 = posHitY[2] ; double z3 = posHitZ[2] ;
        double z2 =  posHitZ[1] ; double x2_MS = posHitX[1] ; double y2_MS = posHitY[1] ; 

        //calculate the position of the hit if there would be no multiple scattering, ie linear extrapolation based on hits position on sensor 3 and 4
        double x2 = z2*(x3-x4)/(z3 - z4) + (x4*z3- x3*z4)/(z3-z4);
        double y2 = z2*(y3-y4)/(z3 - z4) + (y4*z3- y3*z4)/(z3-z4);
        
        //Impact of multiple scattering is therefore given by the difference between the observed hit position ( with multiple scattering ) minus the extrapolated hit position (if there would be no multiple scattering)
        hMultScattX_Pl2->Fill(x2_MS - x2);
        hMultScattY_Pl2->Fill(y2_MS - y2);
    }
    
    if(e){//impact of multiple scattering on sensor 1
        double x3 = posHitX[2] ; double y3 = posHitY[2] ; double z3 = posHitZ[2] ; 
        double x2 = posHitX[1] ; double y2 = posHitY[1] ; double z2 = posHitZ[1] ;
        double z1 =  posHitZ[0] ; double x1_MS = posHitX[0] ; double y1_MS = posHitY[0] ; 

        //calculate the position of the hit if there would be no multiple scattering, ie linear extrapolation based on hits position on sensor 2 and 3
        double x1 = z1*(x2-x3)/(z2 - z3) + (x3*z2- x2*z3)/(z2-z3);
        double y1 = z1*(y2-y3)/(z2 - z3) + (y3*z2- y2*z3)/(z2-z3);
        
        //Impact of multiple scattering is therefore given by the difference between the observed hit position ( with multiple scattering ) minus the extrapolated hit position (if there would be no multiple scattering)
        hMultScattX_Pl1->Fill(x1_MS - x1);
        hMultScattY_Pl1->Fill(y1_MS - y1);
    }
  ...
  cAtLeastHitPerEvent->cd();
  padAtLeastHitPerEvent->cd();
  hAtLeastHitPerEvent->Draw("");
  cAtLeastHitPerEvent->Update();

  cHitsPerEvent->cd();
  padHitsPerEvent->cd();
  hHitsPerEvent->Draw("");
  cHitsPerEvent->Update();
  ...
  ccumulhit3->Write();
  hHitsPerEvent->Write();
  cAtLeastHitPerEvent->Write();
  cMultScatt->Write();
  hMultScattX_Pl1->Write();
  hMultScattY_Pl1->Write();
  hMultScattX_Pl2->Write();
  hMultScattY_Pl2->Write();
  hMultScattX_Pl3->Write();
  hMultScattY_Pl3->Write();
  for( Int_t iPlane=1; iPlane<=nPlanes; iPlane++) {
    tPlane = tTracker->GetPlane(iPlane);
    if( 0 < tPlane->GetAnalysisMode()  ) {
      hHitMap[iPlane-1]->Write();
...
//same thing as SitrineoByEvent but we select only the events with AT LEAST one hit per sensor
void MRaw::Sitrineo_4Hits( Int_t lastPlaneOfFirstTracker)
{
  // Display various info for Sitrineo tracker event by event
  //
  // Argument lastPlaneOfFirsttracker means:
...
// == Plane, hit and track info
  ...
  //Canvas for hits from different planes
  TCanvas *cHitsAlign = new TCanvas("HitsAlign", "HitsAlign", 0, 0, 900, 900);
  TPad *padHits = new TPad("padHits","",0.,0.,1.,0.965);
  padHits->Draw();
  ...
  for( Int_t iPlane=1; iPlane<=nPlanes; iPlane++) { // loop on planes
    ...
    if(gtmp) {
      ...
      hRawData[iPlane-1] = new TH2F(name, title,
                                    Nbins_RawData_U,R_RawData_U[0],R_RawData_U[1],
                                    Nbins_RawData_V,R_RawData_V[0],R_RawData_V[1]);
      hRawData[iPlane-1]->GetXaxis()->SetLabelSize(0.06);
      hRawData[iPlane-1]->GetYaxis()->SetLabelSize(0.06);
      hRawData[iPlane-1]->SetMarkerStyle(20);
      hRawData[iPlane-1]->SetMarkerSize(.4);
      hRawData[iPlane-1]->SetMarkerColor(1);
      hRawData[iPlane-1]->SetStats(kFALSE);
    ...
    if(gtmp) {
      ...
      hTrackMapIndex[iPlane-1] = new TH2F(name, title,
                                          Nbins_RawData_U,R_RawData_U[0],R_RawData_U[1],
                                          Nbins_RawData_V,R_RawData_V[0],R_RawData_V[1]);
      hTrackMapIndex[iPlane-1]->GetXaxis()->SetLabelSize(0.03);
      hTrackMapIndex[iPlane-1]->GetYaxis()->SetLabelSize(0.03);
      hTrackMapIndex[iPlane-1]->SetMarkerStyle(28);
      hTrackMapIndex[iPlane-1]->SetMarkerSize(2.0);
      hTrackMapIndex[iPlane-1]->SetMarkerColor(4);
      hTrackMapIndex[iPlane-1]->SetLineColor(4);
  ...
  for( Int_t iPlane=1; iPlane<=nPlanes; iPlane++) { // loop on planes
    ...
    if(gtmp) {
      ...
      hHitMapTel[iPlane-1] = new TH2F(name, title, 100, xmin, xmax, 100, ymin, ymax);
      hHitMapTel[iPlane-1]->GetXaxis()->SetLabelSize(0.03);
      hHitMapTel[iPlane-1]->GetYaxis()->SetLabelSize(0.03);
      hHitMapTel[iPlane-1]->SetMarkerStyle(20);
      hHitMapTel[iPlane-1]->SetMarkerSize(.6);
      hHitMapTel[iPlane-1]->SetMarkerColor(1);
      hHitMapTel[iPlane-1]->SetStats(kFALSE);
    ...
    if(gtmp) {
      ...
      hHitFoundMapTel[iPlane-1]->SetName( name);
      hHitFoundMapTel[iPlane-1]->SetTitle( title);
      hHitFoundMapTel[iPlane-1]->GetXaxis()->SetLabelSize(0.03);
      hHitFoundMapTel[iPlane-1]->GetYaxis()->SetLabelSize(0.03);
      hHitFoundMapTel[iPlane-1]->SetMarkerStyle(2);
      hHitFoundMapTel[iPlane-1]->SetMarkerSize(1.);
      hHitFoundMapTel[iPlane-1]->SetMarkerColor(2);
      hHitFoundMapTel[iPlane-1]->SetStats(kFALSE);
    ...
    if(gtmp) {
      ...
      hTrackMapTel[iPlane-1]->SetName( name);
      hTrackMapTel[iPlane-1]->SetTitle( title);
      hTrackMapTel[iPlane-1]->GetXaxis()->SetLabelSize(0.03);
      hTrackMapTel[iPlane-1]->GetYaxis()->SetLabelSize(0.03);
      hTrackMapTel[iPlane-1]->SetMarkerStyle(24);
      hTrackMapTel[iPlane-1]->SetMarkerSize(2.);
      hTrackMapTel[iPlane-1]->SetStats(kFALSE);
    }
  ...
  // Track histograms
  TH2F *hTrackMapXY = new TH2F( "htrackmapxy", "Tracks in XY", 100, xmin, xmax, 100, ymin, ymax);
  hTrackMapXY->GetXaxis()->SetLabelSize(0.03);
  hTrackMapXY->GetYaxis()->SetLabelSize(0.03);
  hTrackMapXY->SetXTitle("X (#mum)");
  hTrackMapXY->SetYTitle("Y (#mum)");
  TH2F *hTrackMapXZ = new TH2F( "htrackmapxz", "Tracks in XZ", 100, xmin, xmax, 100, zmin, zmax);
  hTrackMapXZ->GetXaxis()->SetLabelSize(0.03);
  hTrackMapXZ->GetYaxis()->SetLabelSize(0.03);
  hTrackMapXZ->SetXTitle("X (#mum)");
  hTrackMapXZ->SetYTitle("Z (#mum)");
  TH2F *hTrackMapYZ = new TH2F( "htrackmapyz", "Tracks in YZ", 100, ymin, ymax, 100, zmin, zmax);
  hTrackMapYZ->GetXaxis()->SetLabelSize(0.03);
  hTrackMapYZ->GetYaxis()->SetLabelSize(0.03);
  hTrackMapYZ->SetXTitle("Y (#mum)");
  hTrackMapYZ->SetYTitle("Z (#mum)");
  TH3F *hTrackMapXYZ = new TH3F( "htrackmapxyz", "Tracks in XYZ", 100, xmin, xmax, 100, ymin, ymax, 100, zmin, zmax);
  hTrackMapXYZ->GetXaxis()->SetLabelSize(0.03);
  hTrackMapXYZ->GetYaxis()->SetLabelSize(0.03);
  hTrackMapXYZ->GetZaxis()->SetLabelSize(0.03);
  hTrackMapXYZ->SetXTitle("X (#mum)");
  hTrackMapXYZ->SetYTitle("Y (#mum)");
  hTrackMapXYZ->SetZTitle("Z (#mum)");
  ...
  //Select the event with at least one hit per sensor
  bool b = false ; //if false, there is not at least one hit per sensor
  int s[nPlanes] ;
  
  while(!b){
      // if at the end of the data file, stop
      if( !(fSession->NextRawEvent()) ) break;
      ...
      //switch FourHits to true, if there is not at least one hit on each sensor, FourHits will switch to false and the loop will continue 
      b = true ;
      for( Int_t iPlane=1; iPlane<=nPlanes; iPlane++) {
          tPlane = tTracker->GetPlane(iPlane);
          s[iPlane-1] = tPlane->GetHitsN() ;
  ...
  for( Int_t iPlane=1; iPlane<=nPlanes; iPlane++) { // loop on planes
    ...
    Int_t a = tPlane->GetHitsN() ;
    if(fVerbose) cout << "Plane " << iPlane << " has " << a << " hits reconstructed" << endl;
    int color_counter = 0;
    for( Int_t iHit=1; iHit<=tPlane->GetHitsN(); iHit++) { //loop on hits (starts at 1 !!)
      aHit = (DHit*)tPlane->GetHit( iHit);
//        printf("Getting seed index for hit %d (address %x) at plane %d\n", iHit, aHit, iPlane);
...
//        printf(" hit %d, (col,row)=(%3d, %3d), (u,v)=(%.1f, %.1f), Npixels=%d, testMult=%d\n", iHit, (int)TheCol, (int)TheRow, u, v, Npixels_in_Hit, testMult);
    ...
    padHits->cd();
    if(iPlane == 1){
        hHitMapTel[iPlane-1]->SetMarkerSize(3);
        hHitMapTel[iPlane-1]->DrawCopy("PMC");
    }
    else{
        hHitMapTel[iPlane-1]->SetMarkerSize(3);
        hHitMapTel[iPlane-1]->DrawCopy("same PMC");
    }
    hHitMapTel[iPlane-1]->Delete();
    hHitFoundMapTel[iPlane-1]->Delete();
    hTrackMapTel[iPlane-1]->Delete();
